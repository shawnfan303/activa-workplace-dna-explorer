...
export default function DiscoveryAssistantPage() {
  ...
  return (
    <div className="bg-aurora-soft">
      <section className="border-b border-aurora-line bg-white">
        <div className="aurora-container py-10 md:py-14">
          <div>
            <h1 className="text-5xl font-semibold leading-[1.02] text-aurora-ink md:text-7xl xl:whitespace-nowrap">AURORA Discovery Assistant</h1>
            <p className="mt-6 max-w-[1120px] text-base leading-8 text-aurora-graphite md:text-lg">
              以公開資料與顧問式問診邏輯，協助營業快速完成拜訪前準備。正式流程分為需求條件、問診建議與拜訪摘要三段，協助團隊把客戶討論從家具品項提升到工作場域策略。
            </p>
            <div className="mt-9 flex flex-wrap items-center gap-3">
              <button type="button" onClick={() => goToStep("input")} className="inline-flex h-11 items-center justify-center rounded-[6px] bg-aurora-red px-5 text-sm font-semibold text-white shadow-subtle transition duration-200 hover:bg-red-800">開始需求探索</button>
              <div className="inline-flex h-11 min-w-[210px] items-center justify-center rounded-[6px] border border-aurora-line bg-white px-5 shadow-subtle">
      ...
      <div className="aurora-container py-7">
        <div className="grid gap-2 rounded-[10px] border border-aurora-line bg-white p-2 shadow-subtle md:grid-cols-3">
          {stepItems.map((item) => (
            <button key={item.id} type="button" onClick={() => goToStep(item.id)} className={`rounded-[7px] px-4 py-3 text-left text-sm font-semibold transition duration-200 ${flowStep === item.id ? "bg-aurora-red text-white" : "bg-aurora-soft text-aurora-graphite hover:text-aurora-red"}`}>
              {item.label}
            </button>
          ))}
        </div>
      ...
      {flowStep === "input" ? (
        <div className="aurora-container grid gap-8 pb-10 lg:grid-cols-[0.74fr_1.26fr]">
          <div className="space-y-5 lg:sticky lg:top-6 lg:self-start">
            ...
            <button type="button" onClick={resetInput} className="w-full rounded-[6px] border border-aurora-line bg-white px-5 py-3 text-sm font-semibold text-aurora-ink transition duration-200 hover:border-aurora-red hover:text-aurora-red">重設示範條件</button>
          </div>
          <div id="discovery-form" className="space-y-6">
            ...
            <div className="flex justify-end">
              <button type="button" onClick={() => goToStep("results")} className="rounded-[6px] bg-aurora-red px-6 py-3 text-sm font-semibold text-white shadow-subtle transition duration-200 hover:bg-red-800">產生問診建議</button>
            </div>
          </div>
        </div>
      ) : null}
      ...
      {flowStep === "results" ? (
        <div className="aurora-container space-y-8 pb-10">
          {!result.counters.safety.safe ? (
            <section className="rounded-[10px] border border-aurora-red bg-red-50 p-5 text-sm leading-7 text-aurora-red">
              偵測到可能不適合公開系統使用的內容，已阻擋輸出。請移除個資、報價、公司內部資訊或未公開專案資料。
            </section>
          ) : (
            <>
              <section className="rounded-[10px] border border-aurora-line bg-white p-6 shadow-subtle">
                ...
                <div className="mt-5 flex flex-wrap gap-2">
                  <button type="button" onClick={() => goToStep("input")} className="rounded-[6px] border border-aurora-line bg-white px-5 py-3 text-sm font-semibold text-aurora-ink transition duration-200 hover:border-aurora-red hover:text-aurora-red">返回修改條件</button>
                  <button type="button" onClick={markGenerated} className="rounded-[6px] bg-aurora-red px-5 py-3 text-sm font-semibold text-white transition duration-200 hover:bg-red-800">產生拜訪摘要</button>
                </div>
              </section>
        ...
        <div className="aurora-container space-y-6 pb-10">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              <button type="button" onClick={() => goToStep("results")} className="rounded-[6px] border border-aurora-line bg-white px-5 py-3 text-sm font-semibold text-aurora-ink transition duration-200 hover:border-aurora-red hover:text-aurora-red">返回問診建議</button>
              <button type="button" onClick={() => goToStep("input")} className="rounded-[6px] border border-aurora-line bg-white px-5 py-3 text-sm font-semibold text-aurora-ink transition duration-200 hover:border-aurora-red hover:text-aurora-red">重新設定條件</button>
            </div>
          </div>
          <DiscoveryBrief
            input={input}
